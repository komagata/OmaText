...
QtObject {
  ...
  // Standalone adapter: C++ reads the same files without Quickshell.Io.
  function reloadTheme() {
    foreground = "#cacccc"; background = "#101315"; accent = "#cacccc"
    urgent = "#a55555"; muted = "#707880"
    loadColors(themeFiles.colors)
    loadShell(themeFiles.shell)
    loadUserShell(themeFiles.userShell)
  }
  property Connections themeConnection: Connections {
    target: themeFiles
    function onChanged() { root.reloadTheme() }
  }
